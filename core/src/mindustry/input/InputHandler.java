...
public abstract class InputHandler implements InputProcessor, GestureListener {
    ...
    public InputHandler() {
        ...
        Events.on(UnitDestroyEvent.class, e -> {
            if (e.unit != null && e.unit.isPlayer() && e.unit.getPlayer().isLocal()
                    && e.unit.type.weapons.contains(w -> w.bullet.killShooter)) {
    ...
    public static void commandUnits(Player player, int[] unitIds, @Nullable Building buildTarget,
            @Nullable Unit unitTarget, @Nullable Vec2 posTarget) {
    ...
    public static void requestItem(Player player, Building build, Item item, int amount) {
        if (player == null || build == null || !build.interactable(player.team())
                || !player.within(build, itemTransferRange) || player.dead() || amount <= 0)
            return;
    ...
    public static void transferInventory(Player player, Building build) {
        if (player == null || build == null || !player.within(build, itemTransferRange) || build.items == null
                || player.dead() || (state.rules.onlyDepositCore && !(build instanceof CoreBuild)))
            return;
    ...
    public static void requestUnitPayload(Player player, Unit target) {
        if (player == null || !(player.unit() instanceof Payloadc pay))
            return;
    ...
    public static void requestBuildPayload(Player player, Building build) {
        if (player == null || !(player.unit() instanceof Payloadc pay) || build == null)
            return;

        Unit unit = player.unit();

        if (!unit.within(build, tilesize * build.block.size * 1.2f + tilesize * 5f))
            return;
        ...
        if (state.teams.canInteract(unit.team, build.team)) {
            ...
            } else if (build.block.buildVisibility != BuildVisibility.hidden && build.canPickup()
                    && pay.canPickup(build)) {
    ...
    public static void pickedBuildPayload(Unit unit, Building build, boolean onGround) {
        if (build != null && unit instanceof Payloadc pay) {
            if (onGround) {
                if (build.block.buildVisibility != BuildVisibility.hidden && build.canPickup()
                        && pay.canPickup(build)) {
                    pay.pickup(build);
                } else {
    ...
    public static void requestDropPayload(Player player, float x, float y) {
        if (player == null || net.client() || player.dead())
            return;
        ...
        if (net.server()
                && !netServer.admins.allowAction(player, ActionType.dropPayload, player.unit().tileOn(), action -> {
    ...
    public static void rotateBlock(@Nullable Player player, Building build, boolean direction) {
        ...
        if (net.server() && (!Units.canInteract(player, build) ||
                !netServer.admins.allowAction(player, ActionType.rotate, build.tile(),
                        action -> action.rotation = Mathf.mod(build.rotation + Mathf.sign(direction), 4)))) {
    ...
    public static void tileConfig(@Nullable Player player, Building build, @Nullable Object value) {
        if (build == null)
            return;
        if (net.server() && (!Units.canInteract(player, build) ||
                !netServer.admins.allowAction(player, ActionType.configure, build.tile,
                        action -> action.config = value))) {
    ...
    public static void buildingControlSelect(Player player, Building build) {
        ...
        if (net.server()
                && !netServer.admins.allowAction(player, ActionType.buildSelect, action -> action.tile = build.tile)) {
    ...
    /**
     * Adds an input lock; if this function returns true, input is locked. Used for
     * mod 'cutscenes' or custom camera panning.
     */
    ...
    public void update() {
        ...
        if (player.shooting && !wasShooting && player.unit().hasWeapons() && state.rules.unitAmmo
                && !player.team().rules().infiniteAmmo && player.unit().ammo <= 0) {
        ...
        // essentially, this means the client waits for ~1 second after controlling
        // something before trying to control something else automatically
        if (!player.dead() && (recentRespawnTimer -= Time.delta / 70f) <= 0f
                && player.justSwitchFrom != player.unit()) {
            controlledType = player.unit().type;
        }

        if (controlledType != null && player.dead() && controlledType.playerControllable) {
            Unit unit = Units.closest(player.team(), player.x, player.y,
                    u -> !u.isPlayer() && u.type == controlledType && !u.dead);
    ...
    public void checkUnit() {
        if (controlledType != null && controlledType.playerControllable) {
            ...
            if (unit == null && controlledType == UnitTypes.block) {
                unit = world.buildWorld(player.x, player.y) instanceof ControlBlock cont && cont.canControl()
                        ? cont.unit()
                        : null;
            }
    ...
    public void tryPickupPayload() {
        ...
        Unit target = Units.closest(player.team(), pay.x(), pay.y(), unit.type.hitSize * 2f,
                u -> u.isAI() && u.isGrounded() && pay.canPickup(u) && u.within(unit, u.hitSize + unit.hitSize));
        if (target != null) {
    ...
    public void selectUnitsRect() {
        if (commandMode && commandRect) {
            if (!tappedOne) {
                var units = selectedCommandUnits(commandRectX, commandRectY, input.mouseWorldX() - commandRectX,
                        input.mouseWorldY() - commandRectY);
                if (multiUnitSelect()) {
    ...
    public void selectTypedUnits() {
        if (commandMode) {
            ...
            if (unit != null) {
                ...
                selectedUnits.addAll(selectedCommandUnits(Tmp.r1.x, Tmp.r1.y, Tmp.r1.width, Tmp.r1.height,
                        u -> u.type == unit.type));
                Events.fire(Trigger.unitCommandChange);
            }
        }
            ...
            if (selectedUnits.size > 0) {
                ...
                if (ids.length > maxChunkSize) {
                    for (int i = 0; i < ids.length; i += maxChunkSize) {
                        ...
                        Call.commandUnits(player, data, attack instanceof Building b ? b : null,
                                attack instanceof Unit u ? u : null, target);
                    }
                } else {
                    Call.commandUnits(player, ids, attack instanceof Building b ? b : null,
                            attack instanceof Unit u ? u : null, target);
                }
            }
    ...
    public void drawCommand(Unit sel) {
        Drawf.square(sel.x, sel.y, sel.hitSize / 1.4f + Mathf.absin(4f, 1f),
                selectedUnits.contains(sel) ? Pal.remove : Pal.accent);
    }
    ...
    protected void flushSelectPlans(Seq<BuildPlan> plans) {
        for (BuildPlan plan : plans) {
            if (plan.block != null && validPlace(plan.x, plan.y, plan.block, plan.rotation)) {
                ...
                } else if (!other.breaking && other.x == plan.x && other.y == plan.y
                        && other.block.size == plan.block.size) {
    ...
    protected void removeSelection(int x1, int y1, int x2, int y2, boolean flush, int maxLength) {
        ...
        for (int x = 0; x <= Math.abs(result.x2 - result.x); x++) {
            for (int y = 0; y <= Math.abs(result.y2 - result.y); y++) {
                ...
                if (!flush) {
                    tryBreakBlock(wx, wy);
                } else if (validBreak(tile.x, tile.y)
        ...
        // remove build plans
        Tmp.r1.set(result.x * tilesize, result.y * tilesize, (result.x2 - result.x) * tilesize,
                (result.y2 - result.y) * tilesize);
    ...
    boolean tileTapped(@Nullable Building build) {
        ...
        if (build.block.commandable && commandMode) {
            // TODO handled in tap.
            consumed = true;
        } else if (build.block.configurable && build.interactable(player.team())) { // check if tapped block is
                                                                                    // configurable
            consumed = true;
        ...
        if (build.interactable(player.team()) && build.block.consumesTap) {
            consumed = true;
                ...
                && (!consumed || build.block.allowConfigInventory)) {
    ...
    boolean canMine(Tile tile) {
        return !Core.scene.hasMouse()
                ...
                && !((!Core.settings.getBool("doubletapmine") && tile.floor().playerUnmineable)
    ...
    public @Nullable Unit selectedUnit() {
        Unit unit = Units.closest(player.team(), Core.input.mouseWorld().x, Core.input.mouseWorld().y, 40f,
                u -> u.isAI() && u.type.playerControllable);
        if (unit != null) {
            unit.hitbox(Tmp.r1);
            Tmp.r1.grow(6f);
        ...
        if (build instanceof ControlBlock cont && cont.canControl() && build.team == player.team()
                && cont.unit() != player.unit() && cont.unit().isAI()) {
            return cont.unit();
        }
    ...
    public void add() {
        Core.input.getInputProcessors().remove(i -> i instanceof InputHandler
                || (i instanceof GestureDetector && ((GestureDetector) i).getListener() instanceof InputHandler));
    ...
    public void tryDropItems(@Nullable Building build, float x, float y) {
        ...
        if (build != null && build.acceptStack(stack.item, stack.amount, player.unit()) > 0
                && build.interactable(player.team()) &&
    ...
    public void rebuildArea(int x, int y, int x2, int y2) {
        ...
        while (broken.hasNext()) {
            ...
            if (block.bounds(plan.x, plan.y, Tmp.r2).overlaps(Tmp.r1)) {
                player.unit()
                        .addBuild(new BuildPlan(plan.x, plan.y, plan.rotation, content.block(plan.block), plan.config));
            }
        }
    }
    ...
    void iterateLine(int startX, int startY, int endX, int endY, Cons<PlaceLine> cons) {
        ...
        for (int i = 0; i < points.size; i++) {
            ...
            if (block != null && Tmp.r2.setSize(block.size * tilesize)
            ...
            Tmp.r3.setSize(block.size * tilesize).setCenter(point.x * tilesize + block.offset,
                    point.y * tilesize + block.offset);
        }
    }
