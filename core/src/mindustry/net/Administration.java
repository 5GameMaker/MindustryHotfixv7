...
public class Administration {
    ...
    /**
     * All player info. Maps UUIDs to info. This persists throughout restarts. Do
     * not modify directly.
     */
        ...
        addChatFilter((player, message) -> {
            ...
            if (Config.antiSpam.bool() && !player.isLocal() && !player.admin) {
                ...
                if (resetTime > 0 && Time.timeSinceMillis(player.getInfo().lastMessageTime) < resetTime) {
                    // supress message
                    player.sendMessage(
                            "[scarlet]You may only send messages every " + Config.messageRateLimit.num() + " seconds.");
                    player.getInfo().messageInfractions++;
                    // kick player for spamming and prevent connection if they've done this several
                    // times
                    if (player.getInfo().messageInfractions >= Config.messageSpamKick.num()
                            && Config.messageSpamKick.num() != 0) {
                ...
                if (message.equals(player.getInfo().lastSentMessage)
                        && Time.timeSinceMillis(player.getInfo().lastMessageTime) < 1000 * 10) {
    ...
    /**
     * @return time at which a player would be pardoned for a kick (0 means they
     *         were never kicked)
     */
     ...
     * This functionality can be used to implement things like swear filters and
     * special commands.
    ...
    /**
     * Add a filter to actions, preventing things such as breaking or configuring
     * blocks.
     */
     ...
     * If there are players who at any point had this IP, they will be UUID banned
     * as well.
     */
    ...
    /**
     * Unbans a player by IP; returns whether this player was banned in the first
     * place.
    ...
    /**
     * Unbans a player by ID; returns whether this player was banned in the first
     * place.
    ...
    public ObjectSet<PlayerInfo> searchNames(String name) {
        ...
        for (PlayerInfo info : playerInfo.values()) {
            if (info.names.contains(n -> n.toLowerCase().contains(name.toLowerCase())
    ...
    /**
     * Server configuration definition. Each config value can be a string, boolean
     * or number.
     * Creating a new Config instance implicitly adds it to the list of server
     * configs. This can be used for custom plugin configuration.
     */
    public static class Config {
        ...
        serverName = new Config("name", "The server name as displayed on clients.", "Server", "servername"),
                desc = new Config("desc", "The server description, displayed under the name. Max 100 characters.",
                        "off"),
                port = new Config("port", "The port to host on.", Vars.port),
                autoUpdate = new Config("autoUpdate",
                        "Whether to auto-update and exit when a new bleeding-edge update arrives.", false),
                showConnectMessages = new Config("showConnectMessages",
                        "Whether to display connect/disconnect messages.", true),
                enableVotekick = new Config("enableVotekick", "Whether votekick is enabled.", true),
                startCommands = new Config("startCommands",
                ...
                strict = new Config("strict",
                        "Whether strict mode is on - corrects positions and prevents duplicate UUIDs.", true),
                antiSpam = new Config("antiSpam", "Whether spammers are automatically kicked and rate-limited.",
                        headless),
                interactRateWindow = new Config("interactRateWindow",
                        "Block interaction rate limit window, in seconds.", 6),
                interactRateLimit = new Config("interactRateLimit", "Block interaction rate limit.", 25),
                interactRateKick = new Config("interactRateKick",
                        "How many times a player must interact inside the window to get kicked.", 60),
                messageRateLimit = new Config("messageRateLimit", "Message rate limit in seconds. 0 to disable.", 0),
                messageSpamKick = new Config("messageSpamKick",
                        "How many times a player must send a message before the cooldown to get kicked. 0 to disable.",
                        3),
                packetSpamLimit = new Config("packetSpamLimit",
                        "Limit for packet count sent within 3sec that will lead to a blacklist + kick.", 300),
                chatSpamLimit = new Config("chatSpamLimit",
                        "Limit for chat packet count sent within 2sec that will lead to a blacklist + kick. Not the same as a rate limit.",
                        20),
                socketInput = new Config("socketInput",
                        "Allows a local application to control this server through a local TCP socket.", false,
                        "socket", () -> Events.fire(Trigger.socketConfigChanged)),
                socketInputPort = new Config("socketInputPort", "The port for socket input.", 6859,
                        () -> Events.fire(Trigger.socketConfigChanged)),
                socketInputAddress = new Config("socketInputAddress", "The bind address for socket input.", "localhost",
                        () -> Events.fire(Trigger.socketConfigChanged)),
                allowCustomClients = new Config("allowCustomClients", "Whether custom clients are allowed to connect.",
                        !headless, "allow-custom"),
                ...
                autosaveAmount = new Config("autosaveAmount",
                        "The maximum amount of autosaves. Older ones get replaced.", 10),
                autosaveSpacing = new Config("autosaveSpacing", "Spacing between autosaves in seconds.", 60 * 5),
                debug = new Config("debug", "Enable debug logging.", false,
                ...
                roundExtraTime = new Config("roundExtraTime",
                        "Time before loading a new map after the gameover, in seconds.", 12),
    ...
    public interface ChatFilter {
        /**
         * @return the filtered message; a null string signals that the message should
         *         not be sent.
         */
        @Nullable
    ...
    public interface ActionFilter {
        /**
         * @return whether this action should be permitted. if applicable, make sure to
         *         send this player a message specify why the action was prohibited.
         */
