...
public class LExecutor {
    ...
    // yes, this is a minor memory leak, but it's probably not significant enough to
    // matter
    ...
    /**
     * @return a Var from this processor, never a global constant. May be null if
     *         out of bounds.
     */
    ...
    public static class UnitBindI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            ...
            // binding to `null` was previously possible, but was too powerful and
            // exploitable
            ...
            } else if (exec.obj(type) instanceof Unit u && (u.team == exec.team || exec.privileged)
            ...
            if (unitObj instanceof Unit unit && ai != null) {
                ...
                if (ai.checkTargetTimer(this)) {
                    ...
                    switch (locate) {
                        ...
                        case building -> {
                            Building b = Geometry.findClosest(unit.x, unit.y,
                                    exec.bool(enemy) ? indexer.getEnemy(unit.team, flag)
                                            : indexer.getFlagged(unit.team, flag));
                    ...
                    if (res != null && res.build != null &&
                            (unit.within(res.build.x, res.build.y, Math.max(unit.range(), buildingRange))
                                    || res.build.team == exec.team)) {
    ...
    public static class UnitControlI implements LInstruction {
        ...
        public static LogicAI checkLogicAI(LExecutor exec, Object unitObj) {
            if (unitObj instanceof Unit unit && unit.isValid() && exec.obj(varUnit) == unit
                    && (unit.team == exec.team || exec.privileged) && unit.controller().isLogicControllable()) {
        ...
        public void run(LExecutor exec) {
            ...
            if (unitObj instanceof Unit unit && ai != null) {
                ai.controlTimer = LogicAI.logicControlTimeout;
                float x1 = World.unconv(exec.numf(p1)), y1 = World.unconv(exec.numf(p2)),
                        d1 = World.unconv(exec.numf(p3));
                ...
                switch (type) {
                    ...
                    case payDrop -> {
                        if (!exec.timeoutDone(unit, LogicAI.transferDelay))
                            return;
                    ...
                    case payTake -> {
                        ...
                        if (unit instanceof Payloadc pay) {
                            // units
                            if (exec.bool(p1)) {
                                Unit result = Units.closest(unit.team, unit.x, unit.y, unit.type.hitSize * 2f,
                                        u -> u.isAI() && u.isGrounded() && pay.canPickup(u)
                                                && u.within(unit, u.hitSize + unit.hitSize * 1.2f));
                                ...
                                if (build != null && build.team == unit.team) {
                                    ...
                                    } else if (build.block.buildVisibility != BuildVisibility.hidden
                                            && build.canPickup() && pay.canPickup(build)) {
                    ...
                    case build -> {
                        if ((state.rules.logicUnitBuild || exec.privileged) && unit.canBuild()
                                && exec.obj(p3) instanceof Block block && block.canBeBuilt()
                                && (block.unlockedNow() || unit.team.isAI())) {
                            int x = World.toTile(x1 - block.offset / tilesize),
                                    y = World.toTile(y1 - block.offset / tilesize);
                            ...
                            if (tile != null
                                    && !(tile.block() == block && tile.build != null && tile.build.rotation == rot)) {
                    ...
                    case getBlock -> {
                        ...
                        if (!unit.within(x1, y1, range)) {
                            ...
                            if (tile == null) {
                                ...
                                Block block = !tile.synthetic() ? (tile.solid() ? Blocks.stoneWall : Blocks.air)
                                        : tile.block();
                                exec.setobj(p3, block);
                                exec.setobj(p4, tile.build != null ? tile.build : null);
                                // Allows reading of ore tiles if they are present (overlay is not air)
                                // otherwise returns the floor
                    ...
                    case itemDrop -> {
                        ...
                        if (exec.obj(p1) == Blocks.air) {
                            ...
                            if (build != null && build.team == unit.team && build.isValid() && dropped > 0
                                    && unit.within(build, logicItemTransferRange + build.block.size * tilesize / 2f)) {
                                if (build.block instanceof StackConveyor)
                                    return;
                    ...
                    case itemTake -> {
                        ...
                        if (build != null && build.team == unit.team && build.isValid() && build.items != null &&
                                exec.obj(p2) instanceof Item item
                                && unit.within(build, logicItemTransferRange + build.block.size * tilesize / 2f)) {
    ...
    public static class WriteI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            ...
            if (from instanceof MemoryBuild mem
                    && (exec.privileged || (from.team == exec.team && !mem.block.privileged)) && address >= 0
                    && address < mem.memory.length) {
    ...
    public static class RadarI implements LInstruction {
        ...
        // radar instructions are special in that they cache their output and only
        // change it at fixed intervals.
        ...
        public RadarI(RadarTarget target1, RadarTarget target2, RadarTarget target3, RadarSort sort, int radar,
                int sortOrder, int output) {
        ...
        public void run(LExecutor exec) {
            ...
            if (base instanceof Ranged r && (exec.privileged || r.team() == exec.team) &&
                ...
                if ((base instanceof Building && (timer.get(30f) || lastSourceBuild != base))
                        || (ai != null && ai.checkTargetTimer(this))) {
                    // if any of the targets involve enemies
                    boolean enemies = target1 == RadarTarget.enemy || target2 == RadarTarget.enemy
                            || target3 == RadarTarget.enemy;
                    boolean allies = target1 == RadarTarget.ally || target2 == RadarTarget.ally
                            || target3 == RadarTarget.ally;
        ...
        void find(Ranged b, float range, int sortDir, Team team) {
            Units.nearby(team, b.x(), b.y(), range, u -> {
                ...
                boolean valid = target1.func.get(b.team(), u) &&
    ...
    public static class OpI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            if (op == LogicOp.strictEqual) {
                ...
                exec.setnum(dest,
                        v.isobj == v2.isobj
                                && ((v.isobj && Structs.eq(v.objval, v2.objval)) || (!v.isobj && v.numval == v2.numval))
                                        ? 1
                                        : 0);
            } else if (op.unary) {
    ...
    public static class DrawI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            ...
            if (Vars.headless || exec.graphicsBuffer.size >= maxGraphicsBuffer)
                return;
            ...
            if (type == LogicDisplay.commandColorPack) {
                ...
                exec.graphicsBuffer
                        .add(DisplayCmd.get(LogicDisplay.commandColor, pack(r), pack(g), pack(b), pack(a), 0, 0));
            } else {
                // add graphics calls, cap graphics buffer size
                exec.graphicsBuffer.add(DisplayCmd.get(type, packSign(exec.numi(x)), packSign(exec.numi(y)),
                        packSign(num1), packSign(exec.numi(p2)), packSign(exec.numi(p3)), packSign(exec.numi(p4))));
            }
        }
                                    ...
                                    : obj instanceof Building build ? build.block.name
                                            : obj instanceof Unit unit ? unit.type.name
                                                    : obj instanceof Enum<?> e ? e.name()
                                                            : obj instanceof Team team ? team.name : "[object]";
        }
    }
            ...
            if (address != -1) {
                ...
                if (op == ConditionOp.strictEqual) {
                    cmp = va.isobj == vb.isobj
                            && ((va.isobj && va.objval == vb.objval) || (!va.isobj && va.numval == vb.numval));
    ...
    public static class PackColorI implements LInstruction {
        ...
        @Override
        public void run(LExecutor exec) {
            exec.setnum(result, Color.toDoubleBits(Mathf.clamp(exec.numf(r)), Mathf.clamp(exec.numf(g)),
                    Mathf.clamp(exec.numf(b)), Mathf.clamp(exec.numf(a))));
        }
    }
    ...
    public static class FetchI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            ...
            switch (type) {
                ...
                case player ->
                    exec.setobj(result, i < 0 || i >= data.players.size || data.players.get(i).unit().isNull() ? null
                            : data.players.get(i).unit());
    ...
    public static class SetBlockI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            ...
            if (tile != null && exec.obj(block) instanceof Block b) {
                ...
                switch (layer) {
                    case ore -> {
                        if ((b instanceof OverlayFloor || b == Blocks.air) && tile.overlay() != b)
    ...
    public static class SpawnUnitI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            ...
            if (exec.obj(type) instanceof UnitType type && !type.hidden && t != null && Units.canCreate(t, type)) {
                ...
                var unit = type.spawn(t, World.unconv(exec.numf(x)) + Mathf.range(0.01f),
                        World.unconv(exec.numf(y)) + Mathf.range(0.01f));
    ...
    public static class SetRuleI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            switch (rule) {
                ...
                case unitHealth, unitBuildSpeed, unitCost, unitDamage, blockHealth, blockDamage, buildSpeed,
                        rtsMinSquad, rtsMinWeight -> {
    ...
    public static class FlushMessageI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            ...
            if (type == MessageType.announce && ui.hasAnnouncement() ||
    ...
    public static class EffectI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            if (type != null) {
                ...
                // limit size so people don't create lag with ridiculous numbers (some
                // explosions scale with size)
                float rot = type.rotate ? exec.numf(rotation) : Math.min(exec.numf(rotation), 1000f);

                type.effect.at(World.unconv(exec.numf(x)), World.unconv(exec.numf(y)), rot, Tmp.c1.fromDouble(col),
                        exec.obj(data));
            }
        }
    }
    ...
    public static class ExplosionI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            ...
            Call.logicExplosion(t, World.unconv(exec.numf(x)), World.unconv(exec.numf(y)),
                    World.unconv(Math.min(exec.numf(radius), 100)), exec.numf(damage), exec.bool(air),
                    exec.bool(ground), exec.bool(pierce));
        }
    }
    ...
    public static class SetRateI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            if (exec.build != null && exec.build.block.privileged) {
                exec.build.ipt = Mathf.clamp(exec.numi(amount), 1,
                        ((LogicBlock) exec.build.block).maxInstructionsPerTick);
    ...
    public static class SpawnWaveI implements LInstruction {
        ...
        public void run(LExecutor exec) {
            ...
            float spawnX = World.unconv(exec.numf(x)),
                    spawnY = World.unconv(exec.numf(y));
            int packed = Point2.pack(exec.numi(x), exec.numi(y));

            for (SpawnGroup group : state.rules.spawns) {
                if (group.type == null || (group.spawn != -1 && group.spawn != packed))
                    continue;
