...
public class BlockIndexer {
    ...
    /**
     * Stores all ore quadrants on the map. Maps ID to qX to qY to a list of tiles
     * with that ore.
     */
    private IntSeq[][][] ores;
    ...
    public boolean eachBlock(@Nullable Team team, float wx, float wy, float range, Boolf<Building> pred,
            Cons<Building> cons) {
    ...
    public Seq<Building> getEnemy(Team team, BlockFlag type) {
        ...
        if (data.isEmpty()) {
            for (Team enemy : Team.all) {
                if (enemy == team || (enemy == Team.derelict && !state.rules.coreCapture))
                    continue;
    ...
    public Building findTile(Team team, float x, float y, float range, Boolf<Building> pred, boolean usePriority) {
        ...
        for (int i = 0; i < breturnArray.size; i++) {
            ...
            if (!pred.get(next) || !next.block.targetable)
                continue;
    ...
    private void process(Tile tile) {
        ...
        if (blocksPresent != null) {
            ...
            if (tile.blockID() < blocksPresent.length)
